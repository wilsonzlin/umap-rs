...
impl Umap {
    ...
    pub fn fit<'a>(
        ...
        let (a, b) = if let (Some(a_val), Some(b_val)) = (self.config.manifold.a, self.config.manifold.b) {
            (a_val, b_val)
        } else {
        ...
        let disconnection_distance = self.config.graph.disconnection_distance
        ...
        let vertices_disconnected = graph.outer_iterator()
            .filter(|row| {
    ...
    fn validate_parameters<'a>(
        ...
        if self.config.graph.set_op_mix_ratio < 0.0 || self.config.graph.set_op_mix_ratio > 1.0 {
            panic!("set_op_mix_ratio must be between 0.0 and 1.0, got {}",
                   self.config.graph.set_op_mix_ratio);
        }

        if self.config.graph.n_neighbors < 2 {
            panic!("n_neighbors must be >= 2, got {}", self.config.graph.n_neighbors);
        }

        // Validate optimization parameters
        if self.config.optimization.repulsion_strength < 0.0 {
            panic!("repulsion_strength cannot be negative, got {}",
                   self.config.optimization.repulsion_strength);
        }

        if self.config.manifold.min_dist > self.config.manifold.spread {
            panic!("min_dist ({}) must be <= spread ({})",
                   self.config.manifold.min_dist, self.config.manifold.spread);
        }

        if self.config.manifold.min_dist < 0.0 {
            panic!("min_dist cannot be negative, got {}", self.config.manifold.min_dist);
        }

        // Validate optimization parameters
        if self.config.optimization.learning_rate < 0.0 {
            panic!("learning_rate must be positive, got {}",
                   self.config.optimization.learning_rate);
        }

        if self.config.n_components < 1 {
            panic!("n_components must be >= 1, got {}", self.config.n_components);
        }

        // Validate array shapes
        if knn_dists.shape() != knn_indices.shape() {
            panic!("knn_dists and knn_indices must have the same shape, got {:?} vs {:?}",
                   knn_dists.shape(), knn_indices.shape());
        }

        if knn_dists.shape()[1] != self.config.graph.n_neighbors {
            panic!("knn_dists has {} neighbors but n_neighbors is {}",
                   knn_dists.shape()[1], self.config.graph.n_neighbors);
        }

        if knn_dists.shape()[0] != n_samples {
            panic!("knn_dists has {} samples but data has {} samples",
                   knn_dists.shape()[0], n_samples);
        }

        if init.shape()[1] != self.config.n_components {
            panic!("init has {} components but n_components is {}",
                   init.shape()[1], self.config.n_components);
        }

        if init.shape()[0] != n_samples {
            panic!("init has {} samples but data has {} samples",
                   init.shape()[0], n_samples);
        }

        // Validate dataset size
        if n_samples <= self.config.graph.n_neighbors {
            panic!("Number of samples ({}) must be > n_neighbors ({})",
